...
export class InputHandler {
    ...
    constructor(engine: Engine, bodyFactory: BodyFactory, debugControl: DebugControl) {
        this.engine = engine;
            ...
            for (const body of bodies) {
                if (!body.isStatic) {
                    this.debugControl.logEvent("Object Removed", {
                        id: body.id,
                        type: body.circleRadius ? "Circle" : body.vertices ? "Polygon" : "Rectangle",
                        position: {
                            x: body.position.x,
                            y: body.position.y,
                        },
    ...
    private handleClick(event: MouseEvent): void {
        ...
        if (bodies.length > 0) {
            ...
            const randomBody = this.bodyFactory.createRandomBody(event.clientX, event.clientY);
