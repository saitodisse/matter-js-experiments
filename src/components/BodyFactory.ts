...
export class BodyFactory {
    ...
    public createCircle(x: number, y: number, radius: number, options: BodyOptions = {}): Matter.Body {
    ...
    public createPolygon(x: number, y: number, sides: number, radius: number, options: BodyOptions = {}): Matter.Body {
        const body = Matter.Bodies.polygon(x, y, sides, radius, {
            restitution: options.restitution || 0.9,
            friction: options.friction || 0.1,
            render: {
                fillStyle: options.render?.fillStyle || Matter.Common.choose([
                    "#4CAF50", "#8BC34A", "#66BB6A", "#43A047", "#388E3C",
                ]),
    ...
    public createRectangle(x: number, y: number, width: number, height: number, options: BodyOptions = {}): Matter.Body {
        const body = Matter.Bodies.rectangle(x, y, width, height, {
            restitution: options.restitution || 0.9,
            friction: options.friction || 0.1,
            render: {
                fillStyle: options.render?.fillStyle || Matter.Common.choose([
                    "#2196F3", "#64B5F6", "#42A5F5", "#1E88E5", "#1565C0",
                ]),
        ...
        this.debugControl.logEvent("Rectangle Created", {
            ...
            type: "Rectangle",
            position: { x: body.position.x, y: body.position.y },
            size: { width: body.bounds.max.x - body.bounds.min.x, height: body.bounds.max.y - body.bounds.min.y },
        });
    ...
    public createRandomBody(x: number, y: number): Matter.Body {
        ...
        if (type < 0.33) {
            ...
            body = this.createPolygon(x, y, sides, Matter.Common.random(20, 50));
        } else {
