...
Example.ballPool = function () {
    ...
    function updateDebugMode() {
        ...
        localStorage.setItem("debugMode", String(debugCheckbox.checked));
                ...
                if (event.buttons > 0) {
                    ...
                    if (button === "Left" && event.ctrlKey) {
                        ...
                        if (
                            ...
                            var randomBody = createRandomBody(
                                event.clientX,
                                event.clientY,
                            );
    ...
    render.canvas.addEventListener("mousedown", function (event) {
        ...
        if (event.button === 2) {
            ...
            if (debugCheckbox.checked) {
                ...
                console.log(
                    `mousedown: ${event.clientX}, ${event.clientY} bodies id: ${
    ...
    function wrap(body, bounds) {
        if (body.position.x < bounds.min.x) {
            Matter.Body.setPosition(body, {
                x: bounds.max.x - (bounds.min.x - body.position.x),
                y: body.position.y,
            });
        } else if (body.position.x > bounds.max.x) {
            Matter.Body.setPosition(body, {
                x: bounds.min.x + (body.position.x - bounds.max.x),
                y: body.position.y,
            });
        }
        ...
        if (body.position.y < bounds.min.y) {
            Matter.Body.setPosition(body, {
                x: body.position.x,
                y: bounds.max.y - (bounds.min.y - body.position.y),
            });
        } else if (body.position.y > bounds.max.y) {
            Matter.Body.setPosition(body, {
                x: body.position.x,
                y: bounds.min.y + (body.position.y - bounds.max.y),
            });
        }
    }
    ...
    render.canvas.addEventListener("mousemove", function (event) {
        ...
        for (var i = 0; i < allBodies.length; i++) {
            ...
            if (distance < repulsionDistance) {
                var direction = Vector.normalise(
                    Vector.sub(body.position, mousePosition),
                );
        ...
        if (debugCheckbox.checked) {
            console.log("Object Created:", {
                ...
                size: type < 0.33
                    ...
                    : type < 0.66
                    ? { sides: body.vertices.length, radius: body.radius }
    ...
    render.canvas.addEventListener("click", function (event) {
        ...
        if (bodies.length > 0) {
            ...
            if (clickedBody) {
                ...
                var distance = Vector.magnitude(
                    Vector.sub(clickedBody.position, mousePosition),
                );
